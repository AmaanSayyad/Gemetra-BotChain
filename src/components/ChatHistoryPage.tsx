...
export const ChatHistoryPage: React.FC<ChatHistoryPageProps> = ({ onSelectSession }) => {
  ...
  const plainPreviewText = (content: string | null): string => {
  ...
  if (loadingSessions) {
    return (
      <div className="h-full min-w-0 w-full flex items-center justify-center">
                  ...
                  {filteredSessions.map((session) => (
                    ...
                    >
                      ...
                      <p className="text-xs sm:text-sm text-gray-600 line-clamp-2 mb-2 sm:mb-3">
                        {plainPreviewText(session.last_message_content)}
                      </p>
                          ...
                          {message.type === 'assistant' ? (
                            <div className="text-sm sm:text-base leading-relaxed">
                              <ReactMarkdown
                                ...
                                components={{
                                  p: ({ ...props }) => <p className="text-gray-700 my-1.5 leading-relaxed first:mt-0 last:mb-0" {...props} />,
                                  strong: ({ ...props }) => <strong className="font-semibold text-gray-900" {...props} />,
                                  ul: ({ ...props }) => <ul className="list-disc list-inside my-2 space-y-1 text-gray-700 ml-2" {...props} />,
                                  ol: ({ ...props }) => <ol className="list-decimal list-inside my-2 space-y-1 text-gray-700 ml-2" {...props} />,
                                  li: ({ ...props }) => <li className="text-gray-700 my-0.5 leading-relaxed" {...props} />,
                                  a: ({ ...props }) => <a className="text-blue-600 hover:text-blue-800 underline break-words" target="_blank" rel="noopener noreferrer" {...props} />,
                                  code: ({ inline, ...props }) =>
                                    inline ? (
                                      <code className="bg-gray-200 text-gray-800 px-1 py-0.5 rounded text-xs sm:text-sm font-mono" {...props} />
                                    ) : (
                                      <code className="block bg-gray-800 text-gray-100 p-2 rounded text-xs sm:text-sm font-mono overflow-x-auto my-2" {...props} />
                                    ),
                                }}
                              >
  ...
  return (
    <div className="h-full min-h-0 min-w-0 w-full flex">
      {/* Chat Sessions List */}
      <div className="w-full min-w-0 sm:w-2/5 md:w-1/3 max-w-md shrink-0 border-r border-gray-200 bg-white flex flex-col">
        {/* Header */}
      ...
      <div className="flex-1 min-w-0 bg-white flex flex-col">
        {selectedSession ? (
          <>
            {/* Chat Header */}
            <div className="p-4 sm:p-6 border-b border-gray-200 min-w-0">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between min-w-0">
                <div className="min-w-0">
                  <h2 className="text-lg sm:text-xl font-bold text-gray-900 truncate">{selectedSession.title}</h2>
              ...
              {loadingMessages ? (
                ...
                selectedSession.messages.map((message) => (
                  <div
                    ...
                    className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}
                  >
                    <div className={`flex items-start space-x-3 max-w-full sm:max-w-3xl min-w-0 ${
                      message.type === 'user' ? 'flex-row-reverse space-x-reverse' : ''
                    }`}>
